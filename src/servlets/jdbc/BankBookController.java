...
public class BankBookController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(userId!=-1&&from==null&&to==null) {
			ArrayList<BankBookPojo> list=BankBookDao.findAllDateWise("2018-03-01", "2018-03-12", userId);
			request.setAttribute("list", list);
			}
		if(userId!=-1&&from!=null&&to!=null) {
		ArrayList<BankBookPojo> list=BankBookDao.findAllDateWise(from, to, userId);
		request.setAttribute("list", list);
		}
